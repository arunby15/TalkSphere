...
export const signup= async(req,res)=>{
    ...
    try{
       ...
       {
        return res.status(404).json({messgae:"Password must be at least 6 characters"});
       }

       const user = await User.findOne({email})

       if(user) return res.status(400).json({messgae:"Email alread exits."});
...
export const logout=(req,res)=>{
    try{
        ...
        console.log("Error in logout constroller",error.messgae);
    }
};
...
export const updateProfile= async(req,res)=>{
       try{
            ...
            if(!profilePic){
                return res.status(404).json({messgae:"Profile pic is required"});
            }
